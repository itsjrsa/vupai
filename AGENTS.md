...
**Cross-pane activity (Layer 1):** before editing a file, check
`.vupai/activity.current.json` at the git tree root for a sibling pane already
touching it (or run `vupai activity`, or say `activity` / "who's editing"). The
ledger is best-effort and pull-only: it never blocks or injects. `.vupai/` is
auto-gitignored.

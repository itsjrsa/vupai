...
- `vupai keys` - no-arg, interactive: prints the current push-to-talk key(s), then runs the re-runnable picker (`_prompt_hotkey_setup`). Build a **list** of dictation keys and a **list** of command keys via `_select_ptt_keys`: toggle keys into a fresh selection from a curated menu (`hotkey.PTT_KEYS`), by exact pynput key name, or by **pressing the key** (`p` → `hotkey.capture_key`, a one-shot pynput listen); `done`/bare Enter finishes and an empty selection keeps the current keys. Any one key in a list triggers that action (aliases for swapping keyboards). The picker **refuses a key shared by both actions** (the command picker excludes the dictation keys). Persists `hotkey`/`command_hotkey` (as TOML arrays) via `config.set_hotkey_config` (a merge writer preserving comments/other keys); a running daemon needs `vupai reload` to apply. NOTE: macOS pynput collapses the left modifiers (`alt_l`→`alt`, `cmd_l`→`cmd`, `ctrl_l`→`ctrl`), so capture reports those bare names and `PTT_KEYS` uses them for the left-side entries.
...
| `src/vupai/hotkey.py` (`MultiHotkey`) | one pynput listener over N PTT keys, each independently debounced. Binds every dictation key to the dictation callbacks and every system key to the command callbacks. `_make_hotkey` falls back to the default keys (dictation `alt_r` + system `cmd_r`) on overlap/empty/unknown keys |
...
- **Two keys (dictation + system):** the `hotkey` (dictation) injects verbatim
  into the focused pane (no parse, no name routing), and the `command_hotkey`
  (system, default Right-Command) runs the command layer (no control word; a
  non-command falls through to name/number routing, but an **unaddressed**
  utterance — one that hits the focus fallback — is rejected, not injected, so the
  system key never duplicates the dictation key's verbatim-to-focused write). The
  daemon threads a per-utterance `mode` (`"dictation"`/`"system"`) through its jobs
  queue as `(wav, mode)`. Bad key config (duplicate or unknown key names) falls
  back to the default keys (`alt_r`/`cmd_r`) with a feedback error.
- **No control word.** Commands are signalled by the *key* (the system
  key), not a spoken magic word - `control_word` was removed. Only `broadcast_word`
  (default "everyone") still leads as a spoken word. Subset broadcast
...
- **Vocative filler peel (exact-anchor).** Leading fillers ("okay Atlas ...", "um
  ...
  pane 2; (2) `parse_command` peels before the **verb** parse, but NOT
  ...
  so plain dictation that starts with a filler is never corrupted). The
...
- **Command layer runs before the router.** `daemon._process` calls `handle_command`
  after transcribe; the system key's utterance is parsed as a command
  ...
  Interpretation (`parse_command`) is
...
- **Bulk "all" ops are session-scoped.** The registry is server-wide (`list-panes -a`,
  ...
  broadcast, and - unlike the session-scoped
...
multi-pane create
tiles the window · two push-to-talk keys: the dictation key keeps `alt_r`
(muscle memory) and the system key defaults to `cmd_r`.

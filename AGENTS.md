...
- **Sessions follow tmux-style verbs** (the session name is always a positional *after* a verb, never bare, so a mistyped subcommand errors instead of silently creating a session). The name defaults to the cwd basename, slugified by `_slugify_session` (tmux forbids `.`/`:`, so `my.app` -> `my-app`) via `_resolve_session_name`; each directory thus gets its own session. The daemon and the tmux server are **global/server-wide** (routing uses `list-panes -a`, callsigns unique across the server), so one daemon serves all sessions. `ensure_up(name)` creates the session with `-c <cwd>` only when `has_session` is false, then returns the resolved name. `_enter_session` attaches by exact target (`=name`), or `switch-client`s when already inside tmux (so the verbs hop between sessions). `ensure_up` also enables `set-titles` with `set-titles-string "vupai - #S"` (`tmuxio.set_terminal_title`), so the terminal tab reads `vupai - <session>` instead of the bare launch command.
  - `vupai [--reload]` (default, no subcommand) - attach-or-create the **cwd-named** session. `--reload` respawns the daemon first (= `vupai reload && vupai`) so source edits load before attaching - the dogfooding loop. **Inside a tmux pane it skips the attach** (`tmuxio.inside_tmux()` / `$TMUX`): `tmux attach` refuses to nest, so `_cmd_default` respawns the daemon, prints a note, and returns - `--reload` from within degrades to a plain `reload`. Use bare `vupai reload` (no attach) as the in-pane dogfooding loop.
